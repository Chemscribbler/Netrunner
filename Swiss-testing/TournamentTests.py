...
class TestTournamentMethods(unittest.TestCase):
    ...
    def test_constant_penalties(self):
        p1 = Player()
        p2 = Player()
    ...
    def test_sorting_function(self):
        ...
        t.choose_next_pairing_group()
        t.construct_network()
        self.assertEqual(list(t._active_pairing_graph.nodes), [p2])
        t.choose_next_pairing_group()
        t.construct_network()
        self.assertEqual(list(t._active_pairing_graph.nodes), [p3, p1, p4])

    def test_constant_weights_ouptuts(self):
        t = Tournament()
        p1 = Player()
        p2 = Player()
        active_group = (p1, p2)
        p1_index = 0
        p2_index = 1
        self.assertEqual(t._constant_weights(active_group, p1_index,p2_index),t._compute_new_floater_penalty(p1_index, p2_index))

        p1.score = 1

        self.assertEqual(t._constant_weights(active_group, p1_index, p2_index),
                         t._compute_old_floater_penalty(p1, p2, p1_index, p2_index)+t._compute_new_floater_penalty(p1_index,p2_index))
        
        p1.side_balance = 1
        p2.side_balance = 2
        self.assertEqual(t._constant_weights(active_group, p1_index, p2_index),
                         t._compute_old_floater_penalty(p1, p2, p1_index, p2_index)+t._compute_new_floater_penalty(p1_index,p2_index)+t._compute_side_penalty(p1, p2))
